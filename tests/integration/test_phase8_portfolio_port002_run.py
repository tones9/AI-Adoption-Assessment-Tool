...
def test_port002_output_bundle_is_hash_frozen() -> None:
...
def test_port002_candidate_is_live_schema_valid_and_evidence_resolves() -> None:
    ...
    assert len(extraction.candidate.steps) == 6
    assert len(extraction.provider_invocations) == 1
    assert extraction.provider_invocations[0].attempt == 1
    assert [item.code for item in extraction.issues] == ["ambiguous-dependency"]
    validate_candidate_against_document(extraction.candidate, ingestion.document)


def test_port002_review_assessment_and_package_are_complete() -> None:
    ...
    assert len(approval.approved.business_process.steps) == 6
    assert all(not item.retained for item in approval.approved.review.steps[2].dependencies)
    assert all(item.status.value == "resolved" for item in approval.approved.review.conflicts)
    ...
    assert [item.value for item in assessments[2].capabilities] == ["CLASSIFICATION"]
    assert [item.value for item in assessments[3].capabilities] == [
        "WORKFLOW_AUTOMATION"
    ]
    assert [item.value for item in assessments[4].capabilities] == [
        "WORKFLOW_AUTOMATION"
    ]
    assert len(package.package.portfolio.items) == 6
    assert package.package.future_state.status.value == "PROPOSED / NOT DEPLOYED"


def test_port002_sqlite_lineage_is_exact_and_integrity_valid() -> None:
...
def test_port002_after_packet_was_sealed_at_output_freeze() -> None:
    manifest = json.loads((RUN / "output_freeze_manifest.v0.1.json").read_text())
