...
class FrozenEvaluationWorkspaceCompatibilityError(RuntimeError):
    """A frozen workspace cannot be safely read by the current application."""
...
def assert_workspace_write_target_allowed(database_path: str | Path) -> None:
    """Fail closed before an ordinary repository write transaction is opened."""

    if is_frozen_evaluation_portfolio_path(database_path):
        raise FrozenEvaluationWorkspaceError(
            "Writes are refused for frozen evaluation portfolio workspaces"
        )
